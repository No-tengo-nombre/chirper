...
def main(show_fig=False):
    ...
    ax1.plot(*orig_inv.real_part().unpack(), label="Original reconstructed (Re)")
    ax1.plot(*orig_inv.imag_part().unpack(), label="Original reconstructed (Im)")
    ax2.plot(*am_inv.real_part().unpack(), label="AM Modulated reconstructed (Re)")
    ax2.plot(*am_inv.imag_part().unpack(), label="AM Modulated reconstructed (Im)")
    ax3.plot(*pm_inv.real_part().unpack(), label="PM Modulated reconstructed (Re)")
    ax3.plot(*pm_inv.imag_part().unpack(), label="PM Modulated reconstructed (Im)")
    ax1.legend()
    ax2.legend()
    ax3.legend()
    ax1.grid()
    ...
    ax1.plot(*triangle_built.unpack(), label="Original")
    ax2.plot(*orig_inv.real_part().unpack(), label="Original reconstructed (Re)")
    ax2.plot(*orig_inv.imag_part().unpack(), label="Original reconstructed (Im)")
    ax1.legend()
    ax2.legend()
    ax1.grid()
    ax2.grid()
